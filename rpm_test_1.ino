...
void setup()
{
  ...
  attachInterrupt(digitalPinToInterrupt(encoder0PinA), doEncoder0, RISING);  // encoDER ON PIN 2 (Whenever the pin in first argument goes high, doEncoder function is implemented)
  
  pinMode(encoder1PinA, INPUT);
  digitalWrite(encoder1PinA, HIGH);       // turn on pullup resistor
  pinMode(encoder1PinB, INPUT);
  digitalWrite(encoder1PinB, HIGH);       // turn on pullup resistor
  attachInterrupt(digitalPinToInterrupt(encoder1PinA), doEncoder1, RISING);  // encoDER ON PIN 2 (Whenever the pin in first argument goes high, doEncoder function is implemented)
